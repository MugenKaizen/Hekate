...
// packages/core/src/canonical-json.js
...
function validateJsonValue(value, file, path = "", state = { nodes: 0 }) {
  ...
  if (value === null || typeof value === "string" || typeof value === "boolean")
    return [];
  if (typeof value === "number") {
    return Number.isFinite(value) && (!Number.isInteger(value) || Number.isSafeInteger(value)) ? [] : [diagnostic("HKT103", file, `Non-finite or unsafe number at ${path || "/"}.`, path)];
  }
  if (Array.isArray(value)) {
...
function parseYaml(bytes, file) {
  if (bytes.length > MAX_BYTES)
    return { value: null, diagnostics: [diagnostic("HKT009", file, "YAML document exceeds the size limit.")] };
  let source;
  try {
  ...
  if (parserIssues.length > 0) {
    const diagnostics = parserIssues.map((issue) => {
      const code = issue.code === "DUPLICATE_KEY" ? "HKT004" : issue.code === "MULTIPLE_DOCS" ? "HKT007" : String(issue.code).includes("TAG") ? "HKT005" : String(issue.code).includes("ALIAS") ? "HKT006" : "HKT003";
  ...
  if (nonStringKey)
    return { value: null, diagnostics: [diagnostic("HKT008", file, "YAML mapping keys must be strings.")] };
  if (hasAlias)
    return { value: null, diagnostics: [diagnostic("HKT006", file, "YAML aliases are not supported.")] };
  try {
...
var schemasRoot = new URL("../schemas/", import.meta.url);
var schemas = Object.fromEntries(["config", "project", "lock", "manifest", "plan", "install-state", "journal", "prepared-transaction", "apply-state", "legacy-import", "migration-report"].map((name) => [
  name,
  JSON.parse(readFileSync(new URL(`${name}.schema.json`, schemasRoot), "utf8"))
]));
var ajv = new import__2020.default({ strict: true, allErrors: true, ownProperties: true, validateFormats: false });
for (const [name, schema] of Object.entries(schemas))
  ajv.addSchema(schema, name);
var validators = Object.fromEntries(Object.keys(schemas).map((name) => [name, ajv.getSchema(name)]));
var keywordCodes = {
...
function validateSchema(kind, value, file) {
...
// packages/core/src/compiler.js
var CONFIG_PATH = ".workflow/config.yml";
var PROJECT_PATH = ".workflow/project.yml";
var LOCK_PATH = ".workflow/status.lock.json";
var VERSION = "0.1.0";
var profiles = {
...
async function workflowDirectoryExists(root) {
  try {
    ...
    if (!metadata.isDirectory() || metadata.isSymbolicLink())
      throw new UnsafeManagedPathError(".workflow must be a real directory");
    return true;
  } catch (error) {
...
async function optionalBytes(root, relative) {
  ...
  try {
    ...
    if (!metadata.isFile() || metadata.isSymbolicLink())
      throw new UnsafeManagedPathError(`${relative} must be a regular file`);
...
function unknownFacts(project) {
  ...
  return paths.filter(([section, field]) => project[section][field].state === "unknown").map(([section, field]) => `/${section}/${field}`);
}
function verificationCommands(project) {
  ...
  for (const kind of ["format", "lint", "test", "build", "validate"]) {
    ...
    for (const authored of fact.value) {
      ...
      if (!run || /[\r\n\0]/.test(run))
        throw new Error(`Invalid verification command: ${kind}`);
...
async function referenceInputs(root, project) {
  ...
  for (let index = 0;index < fact.value.length; index += 1) {
    ...
    if (seen.has(reference))
      return { inputs: [], diagnostics: [issue("HKT220", PROJECT_PATH, "Duplicate architecture reference.", pointer)] };
    seen.add(reference);
...
async function sourceCandidate(root, overrides = {}) {
  let configBytes;
  try {
    if (!await workflowDirectoryExists(root))
      return { sourceState: "absent", diagnostics: [], candidate: null };
  ...
  try {
    projectBytes = Object.hasOwn(overrides, "projectBytes") ? overrides.projectBytes : await optionalBytes(root, PROJECT_PATH);
  } catch (error) {
    if (!(error instanceof UnsafeManagedPathError))
      throw error;
    projectBytes = Symbol.for("unsafe-project-path");
  }
  if (!configBytes && !projectBytes)
    return { sourceState: "absent", diagnostics: [], candidate: null };
  if (!configBytes)
    return { sourceState: "needs_configuration", diagnostics: [issue("HKT001", CONFIG_PATH, "Required authored config is missing.")], candidate: null };
  const configParsed = parseYaml(configBytes, CONFIG_PATH);
  if (configParsed.diagnostics.length)
    return { sourceState: "invalid", diagnostics: configParsed.diagnostics, candidate: null };
  ...
  if (configDiagnostics.length)
    return { sourceState: "invalid", diagnostics: configDiagnostics, candidate: null };
  const resolved = resolvePolicy(configParsed.value);
  if (resolved.diagnostics.length)
    return { sourceState: "invalid", diagnostics: resolved.diagnostics, candidate: null };
  let sourceState = !configParsed.value.hekate.enabled ? "off" : !configParsed.value.workflow.enabled ? "workflow_disabled" : "ready";
  if (sourceState !== "ready") {
    ...
    return { sourceState, diagnostics: [], candidate: canonicalBytes(lock2), lock: lock2 };
  }
  ...
  if (projectBytes) {
    ...
    if (!projectDiagnostics.length)
      projectDiagnostics = validateSchema("project", project, PROJECT_PATH);
  ...
  if (projectDiagnostics.length && projectBytes)
    return { sourceState: "invalid", diagnostics: projectDiagnostics, candidate: null };
  let references = { inputs: [], diagnostics: [] };
  let commands = [];
  if (project) {
    references = await referenceInputs(root, project);
    if (references.diagnostics.length)
      return { sourceState: "invalid", diagnostics: references.diagnostics, candidate: null };
    if (sourceState === "ready") {
    ...
    try {
      commands = verificationCommands(project);
    } catch {
      return { sourceState: "invalid", diagnostics: [issue("HKT103", PROJECT_PATH, "Verification commands must be non-empty single lines.", "/verification")], candidate: null };
    }
  }
  const inputs = [
...
async function inspect(root) {
  ...
  if (!source.candidate)
    return { ...source, gateState: source.sourceState, lockStatus: "unavailable" };
  let existing;
  try {
    existing = await optionalBytes(root, LOCK_PATH);
  } catch (error) {
    if (!(error instanceof UnsafeManagedPathError))
      throw error;
    return source.sourceState === "ready" ? { ...source, gateState: "invalid", lockStatus: "unsafe", diagnostics: [...source.diagnostics, issue("HKT221", LOCK_PATH, "Generated lock must not be a symlink.")] } : { ...source, gateState: source.sourceState, lockStatus: "unsafe", diagnostics: source.diagnostics };
  }
  if (!existing) {
  ...
  let parsedLock;
  try {
    parsedLock = JSON.parse(existing.toString("utf8"));
  } catch {
    return source.sourceState === "ready" ? { ...source, gateState: "invalid", lockStatus: "invalid", diagnostics: [...source.diagnostics, issue("HKT301", LOCK_PATH, "Generated lock is malformed.")] } : { ...source, gateState: source.sourceState, lockStatus: "invalid", diagnostics: source.diagnostics };
  }
  if (Number.isInteger(parsedLock?.schema_version) && parsedLock.schema_version > 1) {
    return source.sourceState === "ready" ? { ...source, gateState: "unsupported_schema", lockStatus: "unsupported", diagnostics: [...source.diagnostics, issue("HKT303", LOCK_PATH, "Unsupported lock schema version.")] } : { ...source, gateState: source.sourceState, lockStatus: "unsupported", diagnostics: source.diagnostics };
  }
  const lockDiagnostics = validateSchema("lock", parsedLock, LOCK_PATH);
  if (lockDiagnostics.length) {
    return source.sourceState === "ready" ? { ...source, gateState: "invalid", lockStatus: "invalid", diagnostics: [...source.diagnostics, ...lockDiagnostics] } : { ...source, gateState: source.sourceState, lockStatus: "invalid", diagnostics: source.diagnostics };
  }
...
async function atomicWrite(root, bytes) {
  if (!await workflowDirectoryExists(root))
    throw new UnsafeManagedPathError(".workflow directory is missing");
  const destination = path.join(root, LOCK_PATH);
  try {
    const metadata = await lstat(destination);
    if (metadata.isSymbolicLink() || !metadata.isFile())
      throw new UnsafeManagedPathError("Generated lock must be a regular file");
  } catch (error) {
    if (error.code !== "ENOENT")
      throw error;
  }
  ...
  try {
    ...
    handle = null;
    if (!await workflowDirectoryExists(root))
      throw new UnsafeManagedPathError(".workflow directory changed during compilation");
...
async function resolveProjectLock({ root, configBytes, projectBytes }) {
...
async function compileProject({ root, mode = "write" }) {
  const absoluteRoot = path.resolve(root);
  const before = await inspect(absoluteRoot);
  if (mode === "check")
    return { ...before, ok: before.gateState === "ready" && before.lockStatus === "current", wrote: false };
  if (!before.candidate)
    return { ...before, ok: ["absent", "off", "workflow_disabled"].includes(before.gateState), wrote: false };
  ...
  let existing = null;
  try {
    existing = await optionalBytes(absoluteRoot, LOCK_PATH);
  } catch (error) {
    if (!(error instanceof UnsafeManagedPathError))
      throw error;
    return { ...before, ok: false, wrote: false };
  }
  let wrote = false;
...
// packages/core/src/content-materializer.js
import { constants as constants2 } from "node:fs";
import { lstat as lstat3, open as open2, realpath as realpath3 } from "node:fs/promises";
import path3 from "node:path";

// packages/core/src/install-state.js
import { createHash as createHash3 } from "node:crypto";

// packages/core/src/install-planner.js
import { createHash as createHash2 } from "node:crypto";
import { lstat as lstat2, readFile as readFile2, realpath as realpath2 } from "node:fs/promises";
import path2 from "node:path";
var defaultManifestUrl = new URL("../../../distribution/install-manifest.json", import.meta.url);
function diagnostic2(code, file, message, pathValue = "") {
...
function isSafeRelative(value) {
  return typeof value === "string" && !path2.isAbsolute(value) && !value.includes("\\") && value.split("/").every((part) => part && part !== "." && part !== "..");
}
async function inspectFile(root, relative) {
  if (!isSafeRelative(relative))
    throw new Error("path is not a safe project-relative path");
  ...
  return readFile2(current);
}
function validateManifestRelations(manifest) {
  const diagnostics = [];
  const adapters = new Set(manifest.adapters);
  const componentIds = new Set;
  for (const [index, component] of manifest.components.entries()) {
    if (componentIds.has(component.id))
      diagnostics.push(diagnostic2("HKT401", "install-manifest.json", "Duplicate component ID.", `/components/${index}/id`));
    componentIds.add(component.id);
    for (const adapter of component.adapters) {
      if (!adapters.has(adapter))
        diagnostics.push(diagnostic2("HKT402", "install-manifest.json", "Component references an unknown adapter.", `/components/${index}/adapters`));
    }
  }
  ...
  for (const [index, asset] of manifest.assets.entries()) {
    if (assetIds.has(asset.asset_id))
      diagnostics.push(diagnostic2("HKT403", "install-manifest.json", "Duplicate asset ID.", `/assets/${index}/asset_id`));
    assetIds.add(asset.asset_id);
    if (!componentIds.has(asset.component))
      diagnostics.push(diagnostic2("HKT404", "install-manifest.json", "Asset references an unknown component.", `/assets/${index}/component`));
    if (!isSafeRelative(asset.destination))
      diagnostics.push(diagnostic2("HKT405", "install-manifest.json", "Asset destination is unsafe.", `/assets/${index}/destination`));
    if (asset.source !== null && !isSafeRelative(asset.source))
      diagnostics.push(diagnostic2("HKT405", "install-manifest.json", "Asset source is unsafe.", `/assets/${index}/source`));
    const destinationKey = asset.destination.toLowerCase();
    if (destinations.has(destinationKey))
      diagnostics.push(diagnostic2("HKT406", "install-manifest.json", "Asset destinations collide on a case-insensitive filesystem.", `/assets/${index}/destination`));
    destinations.add(destinationKey);
    if (asset.ownership === "generated" && asset.source !== null)
      diagnostics.push(diagnostic2("HKT407", "install-manifest.json", "Generated assets must not have template sources.", `/assets/${index}/source`));
    if (asset.ownership !== "generated" && asset.source === null)
      diagnostics.push(diagnostic2("HKT407", "install-manifest.json", "Non-generated assets require template sources.", `/assets/${index}/source`));
  }
  return diagnostics;
}
function validateInstalledState(state, manifest) {
  ...
  if (state.manifest_version > manifest.manifest_version) {
    diagnostics.push(diagnostic2("HKT420", ".workflow/install-state.json", "Installed state uses a future manifest version.", "/manifest_version"));
  }
  ...
  for (const [index, installed] of state.assets.entries()) {
    if (seen.has(installed.asset_id))
      diagnostics.push(diagnostic2("HKT421", ".workflow/install-state.json", "Duplicate installed asset ID.", `/assets/${index}/asset_id`));
    seen.add(installed.asset_id);
    const expected = manifestAssets.get(installed.asset_id);
    if (!expected) {
      diagnostics.push(diagnostic2("HKT422", ".workflow/install-state.json", "Installed state references an unknown asset.", `/assets/${index}/asset_id`));
    } else if (expected.destination !== installed.destination || expected.ownership !== installed.ownership) {
      diagnostics.push(diagnostic2("HKT423", ".workflow/install-state.json", "Installed asset ownership does not match the manifest.", `/assets/${index}`));
    }
  }
  return diagnostics;
}
async function loadInstallManifest(url = defaultManifestUrl) {
  let manifest;
  try {
    manifest = JSON.parse(await readFile2(url, "utf8"));
  } catch (error) {
    return { manifest: null, diagnostics: [diagnostic2("HKT400", "install-manifest.json", `Cannot read install manifest: ${error.message}`)] };
  }
  const diagnostics = validateSchema("manifest", manifest, "install-manifest.json").map(({ severity: _severity, ...item }) => item);
  if (diagnostics.length === 0)
    diagnostics.push(...validateManifestRelations(manifest));
  return { manifest: diagnostics.length ? null : manifest, diagnostics };
}
async function loadInstalledState(root, manifest) {
  let bytes;
  try {
    bytes = await inspectFile(await realpath2(root), ".workflow/install-state.json");
  } catch (error) {
    return { state: null, diagnostics: [diagnostic2("HKT424", ".workflow/install-state.json", `Cannot safely read installed state: ${error.message}`)] };
  }
  if (bytes === null)
  ...
  try {
    ...
    return { state: null, diagnostics: [diagnostic2("HKT425", ".workflow/install-state.json", "Installed state is malformed JSON.")] };
  }
  const diagnostics = validateInstalledState(state, manifest);
  return { state: diagnostics.length ? null : state, diagnostics };
}
async function planInstallation({ root, sourceRoot, adapters = [], components = [], manifest: suppliedManifest, installedState: suppliedState }) {
  ...
  if (unknownAdapters.length) {
    return { plan: null, diagnostics: unknownAdapters.map((adapter) => diagnostic2("HKT410", "adapters", `Unknown adapter: ${adapter}`)) };
  }
  const requestedComponents = [...new Set(components)].sort();
  const optionalComponentIds = new Set(manifest.components.filter((component) => !component.default && component.adapters.length === 0).map((component) => component.id));
  ...
  if (unknownComponents.length) {
    return { plan: null, diagnostics: unknownComponents.map((component) => diagnostic2("HKT414", "components", `Unknown optional component: ${component}`)) };
  }
  let projectRoot;
  let templatesRoot;
  try {
    projectRoot = await realpath2(root);
    templatesRoot = await realpath2(sourceRoot);
  } catch (error) {
    return { plan: null, diagnostics: [diagnostic2("HKT411", "root", `Cannot resolve planning root: ${error.message}`)] };
  }
  ...
  if (installedState === undefined) {
    ...
    if (loadedState.diagnostics.length)
      return { plan: null, diagnostics: loadedState.diagnostics };
    ...
    if (stateDiagnostics.length)
      return { plan: null, diagnostics: stateDiagnostics };
  }
  ...
  const selectedComponents = manifest.components.filter((component) => component.default || component.adapters.some((adapter) => selectedSet.has(adapter)) || requestedSet.has(component.id)).map((component) => component.id).sort();
  ...
  for (const asset of manifest.assets.filter((item) => componentSet.has(item.component)).sort((left, right) => left.asset_id.localeCompare(right.asset_id, "en"))) {
    ...
    let unsafe = false;
    try {
      currentBytes = await inspectFile(projectRoot, asset.destination);
    } catch (error) {
      diagnostics.push(diagnostic2("HKT412", asset.destination, `Unsafe managed destination: ${error.message}`));
      unsafe = true;
    }
    if (asset.source !== null) {
      try {
        targetBytes = await inspectFile(templatesRoot, asset.source);
      } catch (error) {
        diagnostics.push(diagnostic2("HKT413", asset.source, `Unsafe or missing manifest source: ${error.message}`));
        unsafe = true;
      }
      if (targetBytes === null) {
        diagnostics.push(diagnostic2("HKT413", asset.source, "Manifest source is missing."));
        unsafe = true;
      }
    }
    let disposition;
    const provenance = installedAssets.has(asset.asset_id) ? "recorded" : asset.ownership === "generated" ? "not-applicable" : "unrecorded";
    if (unsafe)
      disposition = "preserve";
    else if (asset.ownership === "user-owned")
      disposition = "preserve";
    else if (asset.update_strategy === "preserve")
      disposition = "preserve";
    else if (asset.update_strategy === "regenerate" && asset.ownership === "generated")
      disposition = "regenerate";
    else if (asset.update_strategy === "structured-merge" && asset.ownership === "shared-merge")
      disposition = "merge";
    else if (currentBytes === null)
      disposition = "create";
    else if (targetBytes !== null && currentBytes.equals(targetBytes))
      disposition = "current";
    else if (asset.update_strategy === "three-way" && asset.ownership === "template-managed")
      disposition = provenance === "recorded" ? "replace" : "replace-candidate";
    else
      disposition = "preserve";
    plannedAssets.push({
      ...
      provenance,
      disposition,
      current_hash: currentBytes === null ? null : hash2(currentBytes),
      target_hash: targetBytes === null ? null : hash2(targetBytes)
    });
  }
  for (const asset of manifest.assets.filter((item) => !componentSet.has(item.component) && installedAssets.has(item.asset_id)).sort((left, right) => left.asset_id.localeCompare(right.asset_id, "en"))) {
    ...
    const currentHash = currentBytes === null ? null : hash2(currentBytes);
    ...
    if (!unsafe && unmodified && ["remove-if-unmodified", "remove-if-generated"].includes(asset.remove_strategy))
      disposition = "remove";
    else if (!unsafe && currentBytes !== null && asset.remove_strategy === "remove-contribution")
      disposition = "merge";
    else if (!unsafe && currentBytes !== null && asset.remove_strategy === "archive")
      disposition = "remove";
    plannedAssets.push({
      asset_id: asset.asset_id,
      destination: asset.destination,
...
// packages/core/src/install-state.js
function diagnostic3(code, message, pathValue = "") {
...
function createInstalledState({ manifest, plan, targetRelease, resolvedContentByAssetId = new Map }) {
  const diagnostics = [
    ...validateSchema("manifest", manifest, "install-manifest.json"),
    ...validateSchema("plan", plan, "install-plan.json")
  ];
  if (!(resolvedContentByAssetId instanceof Map))
    diagnostics.push(diagnostic3("HKT480", "Resolved content must be supplied as a map."));
  if (typeof targetRelease !== "string" || targetRelease.length === 0)
    diagnostics.push(diagnostic3("HKT480", "Target release must be a non-empty string.", "/source_release"));
  if (plan?.manifest_version !== manifest?.manifest_version)
    diagnostics.push(diagnostic3("HKT480", "Plan and manifest versions do not match.", "/manifest_version"));
  if (diagnostics.length)
    return { state: null, bytes: null, diagnostics };
  ...
  for (const [index, asset] of manifest.assets.filter((item) => selected.has(item.component)).sort((left, right) => left.asset_id.localeCompare(right.asset_id, "en")).entries()) {
    ...
    if (!item || item.destination !== asset.destination || item.ownership !== asset.ownership) {
      diagnostics.push(diagnostic3("HKT481", "Selected manifest asset is missing or inconsistent in the plan.", `/assets/${index}`));
      continue;
    }
    const resolved = resolvedContentByAssetId.get(asset.asset_id);
    if (resolved !== undefined && !Buffer.isBuffer(resolved)) {
      diagnostics.push(diagnostic3("HKT481", "Resolved destination content must be bytes.", `/assets/${index}`));
      continue;
    }
    let destinationHash = null;
    if (asset.destination !== ".workflow/install-state.json") {
      if (resolved !== undefined)
        destinationHash = digest(resolved);
      else if (["create", "replace", "replace-candidate"].includes(item.disposition))
        destinationHash = item.target_hash;
      else
        destinationHash = item.current_hash;
    }
    if (destinationHash === null && asset.destination !== ".workflow/install-state.json" && asset.ownership !== "user-owned") {
      diagnostics.push(diagnostic3("HKT481", "Selected asset has no resulting destination hash.", `/assets/${index}/destination_hash`));
    }
    if (asset.source !== null && item.target_hash === null)
      diagnostics.push(diagnostic3("HKT481", "Selected sourced asset has no installed source hash.", `/assets/${index}/installed_source_hash`));
    assets.push({
      asset_id: asset.asset_id,
      destination: asset.destination,
      ownership: asset.ownership,
...
// packages/core/src/content-materializer.js
function diagnostic4(code, message, pathValue = "") {
...
async function readSafe(root, relative, optional = false) {
  ...
  const handle = await open2(current, constants2.O_RDONLY | (constants2.O_NOFOLLOW ?? 0));
  try {
...
` : "");
  ...
  if (missing.length === 0)
    return Buffer.from(currentBytes ?? Buffer.alloc(0));
...
async function materializeInstallationContent({ root, sourceRoot, manifest, plan, targetRelease, imported = null, resolvedContentByAssetId = new Map }) {
  const diagnostics = [
    ...validateSchema("manifest", manifest, "install-manifest.json"),
    ...validateSchema("plan", plan, "install-plan.json")
  ];
  if (!(resolvedContentByAssetId instanceof Map))
    diagnostics.push(diagnostic4("HKT490", "Resolved content must be supplied as a map."));
  if (imported !== null)
    diagnostics.push(...validateSchema("legacy-import", imported, "import.json"));
  if (diagnostics.length)
    return { resolvedContentByAssetId: null, diagnostics };
  let projectRoot;
  let templatesRoot;
  try {
    projectRoot = await realpath3(root);
    templatesRoot = await realpath3(sourceRoot);
  } catch (error) {
    return { resolvedContentByAssetId: null, diagnostics: [diagnostic4("HKT490", `Cannot resolve content roots: ${error.message}`)] };
  }
  ...
  for (const [assetId, bytes] of resolved) {
    if (!Buffer.isBuffer(bytes))
      diagnostics.push(diagnostic4("HKT490", "Resolved content must be bytes.", assetId));
  }
  ...
  if (imported) {
    for (const [destination, value] of [[".workflow/config.yml", imported.config], [".workflow/project.yml", imported.project]]) {
      ...
      if (!item)
        diagnostics.push(diagnostic4("HKT491", "Imported target is absent from the installation plan.", destination));
      else
        resolved.set(item.asset_id, yamlBytes(value));
    }
  }
  for (const [index, item] of plan.assets.entries()) {
    if (item.disposition !== "merge" || resolved.has(item.asset_id))
      continue;
    const asset = manifestById.get(item.asset_id);
    if (!asset || asset.ownership !== "shared-merge" || asset.source === null) {
      diagnostics.push(diagnostic4("HKT491", "Shared merge requires a manifest contribution or explicit bytes.", `/assets/${index}`));
      continue;
    }
    try {
      ...
      diagnostics.push(diagnostic4("HKT491", `Cannot materialize shared contribution: ${error.message}`, `/assets/${index}`));
    }
  }
  async function targetAuthoredBytes(destination) {
    ...
    if (!item)
      return null;
    if (resolved.has(item.asset_id))
      return resolved.get(item.asset_id);
    const asset = manifestById.get(item.asset_id);
    if (["create", "replace", "replace-candidate"].includes(item.disposition))
      return readSafe(templatesRoot, asset.source);
  ...
  if (lockAsset && !resolved.has(lockAsset.asset_id)) {
    try {
      ...
      if (!lock.ok || !lock.bytes)
        diagnostics.push(...lock.diagnostics, diagnostic4("HKT492", "Target authored state cannot produce a valid generated lock.", lockAsset.asset_id));
      else
        resolved.set(lockAsset.asset_id, lock.bytes);
    } catch (error) {
      diagnostics.push(diagnostic4("HKT492", `Cannot materialize generated lock: ${error.message}`, lockAsset.asset_id));
    }
  }
  for (const asset of manifest.assets.filter((item) => item.ownership === "generated" && plan.selected_components.includes(item.component) && ![".workflow/status.lock.json", ".workflow/install-state.json"].includes(item.destination))) {
    if (!resolved.has(asset.asset_id))
      diagnostics.push(diagnostic4("HKT492", "Generated asset requires explicit bytes.", asset.asset_id));
  }
  if (diagnostics.length)
    return { resolvedContentByAssetId: null, diagnostics };
  ...
  if (stateAsset) {
    ...
    if (installed.bytes)
      resolved.set(stateAsset.asset_id, installed.bytes);
  }
...
function diagnostic5(code, message, pathValue = "") {
  return { code, severity: "error", file: "operation-journal.json", path: pathValue, message };
}
function safeRelative(value) {
  if (typeof value !== "string" || path4.isAbsolute(value) || value.includes("\\"))
    return false;
  return value.split("/").every((part) => part && part !== "." && part !== ".." && !part.includes(":") && !/[<>"|?*]/.test(part) && !/[. ]$/.test(part) && !/[\u0000-\u001f]/.test(part) && !/^(con|prn|aux|nul|com[1-9]|lpt[1-9])(?:\.|$)/i.test(part));
}
function createTransactionId(date = new Date, entropy = randomBytes2(8)) {
...
function validateOperationJournal(journal) {
  ...
  for (const [index, operation] of journal.operations.entries()) {
    ...
    if (!safeRelative(operation.path))
      diagnostics.push(diagnostic5("HKT430", "Journal operation path is unsafe.", `${pointer}/path`));
    if (operation.backup_path !== null && !safeRelative(operation.backup_path))
      diagnostics.push(diagnostic5("HKT430", "Journal backup path is unsafe.", `${pointer}/backup_path`));
    const key = operation.path.toLowerCase();
    if (paths.has(key))
      diagnostics.push(diagnostic5("HKT431", "Journal contains duplicate operation paths.", `${pointer}/path`));
    paths.add(key);
    if ([".workflow/backups", ".workflow/migration", ".workflow/transactions"].some((prefix) => operation.path === prefix || operation.path.startsWith(`${prefix}/`)) || operation.path === ".workflow/update.lock") {
      diagnostics.push(diagnostic5("HKT430", "Journal operation targets transaction-owned state.", `${pointer}/path`));
    }
    if (operation.backup_path !== null) {
      const backupKey = operation.backup_path.toLowerCase();
      if (backupPaths.has(backupKey))
        diagnostics.push(diagnostic5("HKT431", "Journal contains duplicate backup paths.", `${pointer}/backup_path`));
      backupPaths.add(backupKey);
    }
    const shapeIsValid = operation.operation === "create" ? operation.before_hash === null && operation.after_hash !== null && operation.backup_path === null : operation.operation === "delete" ? operation.before_hash !== null && operation.after_hash === null && operation.backup_path !== null : operation.before_hash !== null && operation.after_hash !== null && operation.backup_path !== null;
    if (!shapeIsValid)
      diagnostics.push(diagnostic5("HKT432", `Invalid hash or backup fields for ${operation.operation} operation.`, pointer));
    if (["replace", "delete"].includes(operation.operation) && ["local-seed", "project-seed", "shared-merge", "user-owned"].includes(operation.ownership)) {
      diagnostics.push(diagnostic5("HKT433", "Journal operation would replace or delete protected ownership wholesale.", pointer));
    }
    if (operation.ownership === "user-owned")
      diagnostics.push(diagnostic5("HKT433", "Journal operation would mutate user-owned content.", pointer));
  }
  for (const [index, operation] of journal.operations.entries()) {
    if (operation.backup_path !== null && paths.has(operation.backup_path.toLowerCase())) {
      diagnostics.push(diagnostic5("HKT431", "Journal backup path collides with an operation path.", `/operations/${index}/backup_path`));
    }
  }
  return diagnostics;
}
function createOperationJournal({ transactionId = createTransactionId(), manifestVersion, sourceRelease = null, targetRelease, operations }) {
  const journal = {
    schema_version: 1,
...
// packages/core/src/legacy-importer.js
import { createHash as createHash4 } from "node:crypto";
import { constants as constants3 } from "node:fs";
import { lstat as lstat4, open as open3, realpath as realpath4 } from "node:fs/promises";
...
function pointerPart(value) {
  ...
  const safe = KNOWN_KEYS.has(text) ? text : `$key-sha256-${createHash4("sha256").update(text).digest("hex").slice(0, 16)}`;
...
async function safeOptionalFile(root, relative) {
  ...
  const resolved = await realpath4(current);
  if (!resolved.startsWith(`${root}${path5.sep}`))
    throw new Error("path resolves outside the project");
  const handle = await open3(current, constants3.O_RDONLY | (constants3.O_NOFOLLOW ?? 0));
  try {
...
function validateMigrationReport(report) {
  ...
  for (const [index, entry] of report.entries.entries()) {
    ...
    if (entry.disposition === "unresolved" && (entry.target_path !== null || entry.diagnostic_code === null)) {
      diagnostics.push(issue2("HKT516", "report.json", "Unresolved report entry requires a diagnostic and no target.", `/entries/${index}`));
    }
    if (entry.disposition !== "unresolved" && (entry.target_path === null || entry.diagnostic_code !== null)) {
      diagnostics.push(issue2("HKT516", "report.json", "Resolved report entry has inconsistent target fields.", `/entries/${index}`));
    }
  }
  if (Object.keys(counts).some((key) => counts[key] !== report.summary[key])) {
    diagnostics.push(issue2("HKT517", "report.json", "Report summary does not match its entries.", "/summary"));
  }
  return diagnostics;
}
async function importLegacyProject({ root }) {
  const diagnostics = [];
  const documents = {};
  const rawArchives = {};
  const entries = [];
  let projectRoot;
  try {
    projectRoot = await realpath4(root);
  } catch (error) {
    return { imported: null, report: null, importBytes: null, reportBytes: null, diagnostics: [issue2("HKT500", "root", `Cannot resolve import root: ${error.message}`)] };
  }
  for (const [name2, relative, required, defaultDisposition, criticalOverride] of FILES) {
    const critical = criticalOverride ?? required;
    let bytes;
    try {
      bytes = await safeOptionalFile(projectRoot, relative);
    } catch (error) {
      diagnostics.push(issue2("HKT501", relative, `Cannot safely read legacy file: ${error.message}`));
      entries.push({ source_file: relative, source_path: "", source_hash: hash3(Buffer.alloc(0)), target_path: null, disposition: "unresolved", critical, redacted: true, diagnostic_code: "HKT501" });
      continue;
    }
    if (bytes === null) {
      if (required) {
        diagnostics.push(issue2("HKT502", relative, "Required legacy file is missing."));
        entries.push({ source_file: relative, source_path: "", source_hash: hash3(Buffer.alloc(0)), target_path: null, disposition: "unresolved", critical: true, redacted: true, diagnostic_code: "HKT502" });
      }
      continue;
    }
    ...
    if (parsed.diagnostics.length || parsed.value === null || typeof parsed.value !== "object" || Array.isArray(parsed.value)) {
      const parseDiagnostics = parsed.diagnostics.length ? parsed.diagnostics : [issue2("HKT503", relative, "Legacy document root must be a mapping.")];
      const diagnosticCode = parseDiagnostics[0].code;
      entries.push({ source_file: relative, source_path: "", source_hash: hash3(bytes), target_path: null, disposition: "unresolved", critical, redacted: true, diagnostic_code: diagnosticCode });
      if (critical)
        diagnostics.push(...parseDiagnostics);
      else
        rawArchives[`${name2}.yml`] = { encoding: "base64", bytes: bytes.toString("base64"), diagnostic_code: diagnosticCode };
      continue;
    }
    documents[name2] = parsed.value;
    const sourceHash = hash3(bytes);
    const targetBase = defaultDisposition === "preserved" ? `/project/extensions/legacy.hekate/${name2}` : `/archived/${name2}.yml`;
  ...
  const mark = (name2, sourcePath, targetPath, disposition = "transformed") => {
    const relative = FILES.find(([fileName]) => fileName === name2)?.[1];
    for (const entry of entries) {
  ...
  const markExact = (name2, sourcePath, targetPath, disposition = "transformed") => {
    const relative = FILES.find(([fileName]) => fileName === name2)?.[1];
  ...
  const unresolved = (name2, sourcePath, code, message) => {
    const relative = FILES.find(([fileName]) => fileName === name2)?.[1] ?? name2;
    diagnostics.push(issue2(code, relative, message, sourcePath));
    let matched = false;
    for (const entry of entries) {
      if (entry.source_file === relative && (entry.source_path === sourcePath || entry.source_path.startsWith(`${sourcePath}/`))) {
    ...
    if (!matched)
      entries.push({ source_file: relative, source_path: sourcePath, source_hash: hash3(Buffer.alloc(0)), target_path: null, disposition: "unresolved", critical: true, redacted: true, diagnostic_code: code });
  };
  if (diagnostics.length) {
    const report2 = buildReport(entries);
    return { imported: null, report: report2, importBytes: null, reportBytes: canonicalBytes(report2), diagnostics };
  }
  ...
  const booleanAt = (name2, sourcePath, value, fallback) => {
    if (value === undefined)
      return fallback;
    if (typeof value !== "boolean") {
      unresolved(name2, sourcePath, "HKT504", "Legacy boolean setting has the wrong type.");
      return fallback;
    }
    return value;
  };
  ...
  const objectListFact = (name2, sourcePath, value, allowedKeys) => {
    if (value === undefined || Array.isArray(value) && value.length === 0)
      return unknown();
    if (!Array.isArray(value)) {
      unresolved("stack", sourcePath, "HKT506", `Legacy ${name2} must be a list.`);
      return unknown();
    }
    const normalized = [];
    for (const [index, item] of value.entries()) {
      if (typeof item === "string" && item)
        ...
        unresolved("stack", `${sourcePath}/${index}`, "HKT507", `Legacy ${name2} entry requires a non-empty name.`);
    }
    return normalized.length ? known(normalized) : unknown();
  };
  const stringListFact = (name2, sourcePath, value) => {
    if (value === undefined || Array.isArray(value) && value.length === 0)
      return unknown();
    if (!Array.isArray(value) || value.some((item) => typeof item !== "string" || !item)) {
      unresolved(name2, sourcePath, "HKT508", "Legacy list fact must contain non-empty strings.");
      return unknown();
    }
  ...
  const commandFact = (sourcePath, value) => {
    ...
    if (!Array.isArray(commands) || commands.some((command) => typeof command !== "string" || !command || command.includes("\x00") || /[\r\n]/.test(command))) {
  ...
  for (const [name2, sourcePath, value] of profileCandidates) {
    if (typeof value !== "string" || ![...Object.keys(PROFILES), "custom"].includes(value))
      unresolved(name2, sourcePath, "HKT510", "Legacy profile is unsupported.");
  }
  const selectedProfiles = [...new Set(profileCandidates.map(([, , value]) => value).filter((value) => typeof value === "string"))];
  if (selectedProfiles.length > 1) {
    for (const [name2, sourcePath] of profileCandidates)
      unresolved(name2, sourcePath, "HKT511", "Legacy profile owners conflict.");
  }
  const profile = selectedProfiles[0] ?? "custom";
  for (const [name2, sourcePath] of profileCandidates)
    mark(name2, sourcePath, "/config/workflow/profile");
  ...
  if (tdd === undefined) {
    ...
    if (legacyMode !== undefined || legacyEnabled !== undefined) {
      if (legacyEnabled === false || legacyMode === "off")
        tdd = "off";
      else if (legacyEnabled === true && legacyMode === "soft")
        tdd = "prefer-test-first";
      else if (legacyEnabled === true && legacyMode === "strict-lite")
        tdd = "require-test-evidence";
      else
        unresolved("workflow", "/process/light_tdd", "HKT512", "Legacy TDD settings conflict or are unsupported.");
    ...
    mark("workflow", "/process/tdd/mode", "/config/workflow/overrides/tdd/mode");
  ...
  for (const [sourcePath, value] of historyCandidates) {
    if (typeof value !== "boolean")
      unresolved("workflow", sourcePath, "HKT504", "Legacy history setting has the wrong type.");
  }
  const historyValues = [...new Set(historyCandidates.map(([, value]) => value).filter((value) => typeof value === "boolean"))];
  if (historyValues.length > 1) {
    for (const [sourcePath] of historyCandidates)
      unresolved("workflow", sourcePath, "HKT513", "Legacy history owners conflict.");
  }
  const history = historyValues[0];
  for (const [sourcePath] of historyCandidates)
    mark("workflow", sourcePath, "/config/workflow/overrides/history/enabled");
  const overrides = {};
  ...
  if (profile === "custom") {
    ...
    if (tdd !== undefined && tdd !== base.tdd)
      overrides.tdd = { mode: tdd };
    if (history !== undefined && history !== base.history)
  ...
  if (kind.state === "known" && !KINDS.has(kind.value))
    kind = known("other");
  ...
  const markObjectList = (sourcePath, value, targetPath, allowedKeys) => {
    if (Array.isArray(value) && value.length === 0)
      markExact("stack", sourcePath, targetPath);
    if (!Array.isArray(value))
      return;
    let targetIndex = 0;
      ...
      if (typeof item === "string" && item) {
        ...
        for (const key of allowedKeys) {
          if (typeof item[key] === "string" && item[key])
            markExact("stack", `${sourcePath}/${sourceIndex}/${key}`, `${targetPath}/value/${targetIndex}/${key}`);
        }
        targetIndex += 1;
      }
    }
  ...
  if (frameworkGroups !== undefined) {
    if (!frameworkGroups || typeof frameworkGroups !== "object" || Array.isArray(frameworkGroups))
      unresolved("stack", "/frameworks", "HKT506", "Legacy frameworks must be a mapping.");
    else {
      const values = Object.values(frameworkGroups).flatMap((value) => Array.isArray(value) ? value : [value]);
      if (values.some((value) => typeof value !== "string" || !value)) {
        if (values.length)
          unresolved("stack", "/frameworks", "HKT508", "Legacy framework groups must contain non-empty strings.");
      } else if (values.length)
        frameworks = known([...new Set(values)]);
    }
  }
  if (frameworkGroups && typeof frameworkGroups === "object" && !Array.isArray(frameworkGroups)) {
    ...
    for (const [group, values] of Object.entries(frameworkGroups)) {
      if (Array.isArray(values) && values.length === 0)
        markExact("stack", `/frameworks/${group}`, "/project/stack/frameworks");
      if (Array.isArray(values)) {
        for (const [index, value] of values.entries()) {
          if (typeof value === "string" && value)
            markExact("stack", `/frameworks/${group}/${index}`, `/project/stack/frameworks/value/${flattened.indexOf(value)}`);
        }
      }
    }
  }
  ...
  if (Array.isArray(dependencyValues) && dependencyValues.length === 0)
    markExact("stack", "/dependencies", "/project/stack/dependencies");
  if (Array.isArray(dependencyValues)) {
    ...
    for (const [index, value] of dependencyValues.entries()) {
      if (typeof value === "string" && value)
        markExact("stack", `/dependencies/${index}`, `/project/stack/dependencies/value/${uniqueDependencies.indexOf(value)}`);
    }
  }
  const verification = {};
  for (const command of ["format", "lint", "test", "build", "validate"]) {
    ...
    if (typeof value === "string")
      markExact("stack", `/build_and_run/${command}`, `/project/verification/${command}${value ? "/value/0" : ""}`);
    if (Array.isArray(value) && value.length === 0)
      markExact("stack", `/build_and_run/${command}`, `/project/verification/${command}`);
    if (Array.isArray(value))
      value.forEach((_item, index) => markExact("stack", `/build_and_run/${command}/${index}`, `/project/verification/${command}/value/${index}`));
  }
  ...
  if (Array.isArray(dependencyRules) && dependencyRules.length === 0)
    markExact("architecture", "/dependency_rules", "/project/architecture/constraints");
  if (Array.isArray(dependencyRules))
    dependencyRules.forEach((_item, index) => markExact("architecture", `/dependency_rules/${index}`, `/project/architecture/constraints/value/${index}`));
  if (get(documents.status, "schema_version") !== undefined) {
    const statusVersion = get(documents.status, "schema_version");
    if (!Number.isSafeInteger(statusVersion) || statusVersion > 1)
      unresolved("status", "/schema_version", "HKT515", "Legacy status uses an unsupported future schema.");
  }
  for (const name2 of ["orchestration", "orchestration.local"]) {
    const version = get(documents[name2], "schema_version");
    if (version !== undefined && (!Number.isSafeInteger(version) || version > 2))
      unresolved(name2, "/schema_version", "HKT515", "Legacy orchestration uses an unsupported future schema.");
  }
  ...
  const archived = { ...Object.fromEntries(Object.entries(documents).map(([name2, value]) => [`${name2}.yml`, value])), ...rawArchives };
...
// packages/core/src/installation-verifier.js
import { createHash as createHash5 } from "node:crypto";
import { constants as constants4 } from "node:fs";
import { lstat as lstat5, open as open4, realpath as realpath5 } from "node:fs/promises";
import path6 from "node:path";
function diagnostic6(code, file, message, pathValue = "") {
  return { code, severity: "error", file, path: pathValue, message };
}
function digest2(bytes) {
  return `sha256:${createHash5("sha256").update(bytes).digest("hex")}`;
}
async function readSafe2(root, relative) {
  const parts = relative.split("/");
  let current = root;
  for (const [index, part] of parts.entries()) {
  ...
  const handle = await open4(current, constants4.O_RDONLY | (constants4.O_NOFOLLOW ?? 0));
  try {
    return await handle.readFile();
  } finally {
    await handle.close();
...
async function verifyInstalledProject({ root, sourceRoot, manifest, expectedAdapters = [], expectedComponents = [], targetRelease = null, imported = null, report = null }) {
  ...
  if (!Array.isArray(expectedAdapters))
    diagnostics.push(diagnostic6("HKT470", "adapters", "Expected adapters must be an array."));
  if (imported === null !== (report === null))
    diagnostics.push(diagnostic6("HKT470", "migration", "Imported values and preservation report must be supplied together."));
  if (diagnostics.length)
    return { ok: false, diagnostics };
  let projectRoot;
  let templatesRoot;
  try {
    projectRoot = await realpath5(root);
    templatesRoot = await realpath5(sourceRoot);
  } catch (error) {
    return { ok: false, diagnostics: [diagnostic6("HKT470", "root", `Cannot resolve verification root: ${error.message}`)] };
  }
  ...
  if (adapters.length !== expectedAdapters.length || adapters.some((adapter) => !manifest.adapters.includes(adapter))) {
    diagnostics.push(diagnostic6("HKT471", "adapters", "Expected adapters are unknown or duplicated."));
  }
  const requestedComponents = new Set(expectedComponents);
  const selected = manifest.components.filter((component) => component.default || component.adapters.some((adapter) => adapters.includes(adapter)) || !component.default && component.adapters.length === 0 && requestedComponents.has(component.id)).map((component) => component.id).sort();
  ...
  if (!state && loaded.diagnostics.length === 0)
    diagnostics.push(diagnostic6("HKT472", ".workflow/install-state.json", "Installed state is missing."));
  if (state) {
    if (state.manifest_version !== manifest.manifest_version)
      diagnostics.push(diagnostic6("HKT472", ".workflow/install-state.json", "Installed state manifest version is not current.", "/manifest_version"));
    if (targetRelease !== null && state.source_release !== targetRelease)
      diagnostics.push(diagnostic6("HKT472", ".workflow/install-state.json", "Installed state release does not match the transaction target.", "/source_release"));
    if (!sameValues(state.selected_adapters, adapters))
      diagnostics.push(diagnostic6("HKT472", ".workflow/install-state.json", "Installed adapters do not match the requested adapters.", "/selected_adapters"));
    if (!sameValues(state.selected_components, selected))
      diagnostics.push(diagnostic6("HKT472", ".workflow/install-state.json", "Installed components do not match the requested adapters and components.", "/selected_components"));
    ...
    if (!sameValues(stateAssets, expectedAssets))
      diagnostics.push(diagnostic6("HKT472", ".workflow/install-state.json", "Installed asset ledger is incomplete or contains unselected assets.", "/assets"));
    for (const [index, asset] of state.assets.entries()) {
      const manifestAsset = manifest.assets.find((item) => item.asset_id === asset.asset_id);
      if (manifestAsset?.source !== null && asset.installed_source_hash === null)
        diagnostics.push(diagnostic6("HKT473", ".workflow/install-state.json", "Installed source hash is missing from the ownership ledger.", `/assets/${index}/installed_source_hash`));
      if (asset.destination_hash === null) {
        if (manifestAsset && manifestAsset.destination !== ".workflow/install-state.json" && manifestAsset.ownership !== "user-owned")
          diagnostics.push(diagnostic6("HKT473", ".workflow/install-state.json", "Installed destination hash is missing from the ownership ledger.", `/assets/${index}/destination_hash`));
        continue;
      }
      try {
        const bytes = await readSafe2(projectRoot, asset.destination);
        if (digest2(bytes) !== asset.destination_hash)
          diagnostics.push(diagnostic6("HKT473", asset.destination, "Installed asset bytes do not match the ownership ledger.", `/assets/${index}/destination_hash`));
      } catch (error) {
        diagnostics.push(diagnostic6("HKT473", asset.destination, `Cannot verify installed asset: ${error.message}`, `/assets/${index}/destination`));
      }
    }
  }
  const compiler = await compileProject({ root: projectRoot, mode: "check" });
  const safelyGatedInstallation = (imported !== null || state !== null) && compiler.lockStatus === "current" && ["off", "workflow_disabled", "needs_configuration", "needs_confirmation"].includes(compiler.gateState);
  if (!compiler.ok && !safelyGatedInstallation)
    diagnostics.push(...compiler.diagnostics.length ? compiler.diagnostics : [diagnostic6("HKT474", ".workflow/status.lock.json", "Compiler check did not reach an accepted current state.")]);
  if (imported !== null) {
    ...
    if (importDiagnostics.length === 0 && reportDiagnostics.length === 0) {
      if (report.entries.some((entry) => entry.critical && entry.disposition === "unresolved"))
        diagnostics.push(diagnostic6("HKT475", "report.json", "Critical legacy settings remain unresolved.", "/entries"));
      if (!sameValues(imported.selected_adapters, adapters))
        diagnostics.push(diagnostic6("HKT475", "import.json", "Imported adapters do not match the requested adapters.", "/selected_adapters"));
      for (const [relative, expected, kind] of [[".workflow/config.yml", imported.config, "config"], [".workflow/project.yml", imported.project, "project"]]) {
        try {
          const parsed = parseYaml(await readSafe2(projectRoot, relative), relative);
          diagnostics.push(...parsed.diagnostics);
          if (parsed.value) {
            diagnostics.push(...validateSchema(kind, parsed.value, relative));
            const comparableExpected = structuredClone(expected);
            if (kind === "project")
              comparableExpected.confirmation = structuredClone(parsed.value.confirmation);
            if (!canonicalBytes(parsed.value).equals(canonicalBytes(comparableExpected)))
              diagnostics.push(diagnostic6("HKT475", relative, "Migrated target does not preserve the normalized imported values."));
          }
        } catch (error) {
          diagnostics.push(diagnostic6("HKT475", relative, `Cannot verify migrated target: ${error.message}`));
        }
      }
    }
  }
  ...
  if (gitignoreAsset) {
    try {
      const required = (await readSafe2(templatesRoot, gitignoreAsset.source)).toString("utf8").split(/\r?\n/).map((line) => line.trim()).filter((line) => line && !line.startsWith("#"));
      const actual = new Set((await readSafe2(projectRoot, ".gitignore")).toString("utf8").split(/\r?\n/).map((line) => line.trim()).filter(Boolean));
      if (required.some((line) => !actual.has(line)))
        diagnostics.push(diagnostic6("HKT476", ".gitignore", "Required Hekate local-artifact ignore entries are missing."));
    } catch (error) {
      diagnostics.push(diagnostic6("HKT476", ".gitignore", `Cannot verify local-artifact ignore rules: ${error.message}`));
    }
  }
  return { ok: diagnostics.length === 0, compiler, state, diagnostics };
}
// packages/core/src/operation-resolver.js
import { createHash as createHash6 } from "node:crypto";
import { lstat as lstat6, open as open5, realpath as realpath6 } from "node:fs/promises";
import { constants as constants5 } from "node:fs";
import path7 from "node:path";
function diagnostic7(code, message, pathValue = "") {
  return { code, severity: "error", file: "install-plan.json", path: pathValue, message };
}
function digest3(bytes) {
  return `sha256:${createHash6("sha256").update(bytes).digest("hex")}`;
}
function safeRelative2(value) {
  if (typeof value !== "string" || path7.isAbsolute(value) || value.includes("\\"))
    return false;
  return value.split("/").every((part) => part && part !== "." && part !== ".." && !/[:<>"|?*]/.test(part) && !/[. ]$/.test(part) && !/[\u0000-\u001f]/.test(part) && !/^(con|prn|aux|nul|com[1-9]|lpt[1-9])(?:\.|$)/i.test(part));
}
var protectedOwnership = ["local-seed", "project-seed", "shared-merge", "user-owned"];
...
async function readSafeSource(root, relative) {
  if (!safeRelative2(relative))
    throw new Error("source is not a safe relative path");
  let current = root;
  for (const [index, part] of relative.split("/").entries()) {
    current = path7.join(current, part);
    const metadata = await lstat6(current);
    if (metadata.isSymbolicLink())
      throw new Error("source path contains a symbolic link");
    if (index < relative.split("/").length - 1 && !metadata.isDirectory())
      throw new Error("source parent is not a directory");
    if (index === relative.split("/").length - 1 && !metadata.isFile())
      throw new Error("source is not a regular file");
  }
  const handle = await open5(current, constants5.O_RDONLY | (constants5.O_NOFOLLOW ?? 0));
  try {
    return await handle.readFile();
  } finally {
    await handle.close();
...
async function resolveInstallationOperations({
  sourceRoot,
  plan,
  manifest,
  transactionId,
  ...
  if (!(resolvedContentByAssetId instanceof Map))
    diagnostics.push(diagnostic7("HKT460", "Resolved asset content must be supplied as a map."));
  if (!Array.isArray(approvedReplacementCandidates))
    diagnostics.push(diagnostic7("HKT460", "Replacement candidate approvals must be an array."));
  if (plan?.diagnostics?.length)
    diagnostics.push(diagnostic7("HKT460", "A plan with diagnostics cannot be resolved.", "/diagnostics"));
  if (plan?.manifest_version !== manifest?.manifest_version)
    diagnostics.push(diagnostic7("HKT461", "Plan and manifest versions do not match.", "/manifest_version"));
  if (diagnostics.length)
    return { journal: null, contentByPath: null, modeByPath: null, diagnostics };
  const assets = new Map(manifest.assets.map((asset) => [asset.asset_id, asset]));
  if (assets.size !== manifest.assets.length)
    diagnostics.push(diagnostic7("HKT461", "Manifest asset IDs are not unique."));
  const plannedIds = new Set(plan.assets.map((asset) => asset.asset_id));
  if (plannedIds.size !== plan.assets.length)
    diagnostics.push(diagnostic7("HKT461", "Plan asset IDs are not unique."));
  ...
  for (const component of plan.selected_components) {
    if (optionalComponents.has(component))
  ...
  if (expectedComponents.size !== plan.selected_components.length || plan.selected_components.some((component) => !expectedComponents.has(component))) {
    diagnostics.push(diagnostic7("HKT461", "Selected components do not match selected adapters and optional components.", "/selected_components"));
  }
  for (const asset of manifest.assets.filter((item) => expectedComponents.has(item.component))) {
    if (!plannedIds.has(asset.asset_id))
      diagnostics.push(diagnostic7("HKT461", "Plan omits an asset from a selected component.", asset.asset_id));
  }
  for (const assetId of resolvedContentByAssetId.keys()) {
    if (!plannedIds.has(assetId))
      diagnostics.push(diagnostic7("HKT461", "Resolved content references an asset outside the plan.", assetId));
  }
  const approved = new Set(approvedReplacementCandidates);
  for (const assetId of approved) {
    if (!plannedIds.has(assetId))
      diagnostics.push(diagnostic7("HKT461", "Replacement approval references an asset outside the plan.", assetId));
    else if (plan.assets.find((asset) => asset.asset_id === assetId).disposition !== "replace-candidate")
      diagnostics.push(diagnostic7("HKT461", "Replacement approval does not reference a replacement candidate.", assetId));
  }
  let templatesRoot;
  try {
    templatesRoot = await realpath6(sourceRoot);
  } catch (error) {
    return { journal: null, contentByPath: null, modeByPath: null, diagnostics: [diagnostic7("HKT462", `Cannot resolve template root: ${error.message}`)] };
  }
  const operations = [];
  ...
  for (const [index, planned] of plan.assets.entries()) {
    ...
    if (!asset || asset.destination !== planned.destination || asset.ownership !== planned.ownership) {
      diagnostics.push(diagnostic7("HKT461", "Planned asset does not match the manifest.", pointer));
      continue;
    }
    ...
    if (!selected && !["remove", "merge", "preserve"].includes(planned.disposition)) {
      diagnostics.push(diagnostic7("HKT461", "Planned asset belongs to an unselected component.", pointer));
      continue;
    }
    if (!selected && planned.provenance !== "recorded") {
      diagnostics.push(diagnostic7("HKT467", "Unselected assets require recorded installation provenance.", pointer));
      continue;
    }
    if (planned.disposition === "remove") {
      if (selected || planned.provenance !== "recorded" || planned.current_hash === null || planned.target_hash !== null || !["remove-if-unmodified", "remove-if-generated", "archive"].includes(asset.remove_strategy)) {
        diagnostics.push(diagnostic7("HKT467", "Asset is not eligible for safe removal.", pointer));
        continue;
      }
      if (asset.remove_strategy === "archive") {
        ...
        if (protectedOwnership.includes(planned.ownership)) {
          diagnostics.push(diagnostic7("HKT468", "Archived removal requires an ownership class that permits deletion.", pointer));
          continue;
        }
        if (!Buffer.isBuffer(archiveBytes)) {
          diagnostics.push(diagnostic7("HKT468", "Archived removal requires the current bytes of the asset.", pointer));
          continue;
        }
        if (digest3(archiveBytes) !== planned.current_hash) {
          diagnostics.push(diagnostic7("HKT468", "Archived bytes do not match the recorded current content.", pointer));
          continue;
        }
        if (!safeRelative2(archivePath) || contentByPath.has(archivePath)) {
          diagnostics.push(diagnostic7("HKT468", "Archive destination is unsafe or already claimed.", pointer));
          continue;
        }
        operations.push({
          operation: "create",
          path: archivePath,
          before_hash: null,
          after_hash: digest3(archiveBytes),
          backup_path: null,
    ...
    if (!selected && planned.disposition === "merge" && (asset.remove_strategy !== "remove-contribution" || planned.ownership !== "shared-merge" || planned.current_hash === null || planned.target_hash !== null || !resolvedContentByAssetId.has(planned.asset_id))) {
      diagnostics.push(diagnostic7("HKT467", "Contribution removal requires explicit merged bytes for a recorded shared asset.", pointer));
      continue;
    }
    if (!selected && planned.disposition === "preserve" && resolvedContentByAssetId.has(planned.asset_id)) {
      diagnostics.push(diagnostic7("HKT467", "Preserved assets from unselected components cannot be mutated.", pointer));
      continue;
    }
    if (["current", "preserve"].includes(planned.disposition) && !resolvedContentByAssetId.has(planned.asset_id))
      continue;
    if (resolvedContentByAssetId.has(planned.asset_id) && ["local-seed", "user-owned"].includes(planned.ownership)) {
      diagnostics.push(diagnostic7("HKT465", "Resolved content cannot mutate local-seed or user-owned assets.", pointer));
      continue;
    }
    if (asset.update_strategy === "preserve") {
      diagnostics.push(diagnostic7("HKT468", "Assets with a preserve update strategy are never written.", pointer));
      continue;
    }
    if (asset.update_strategy === "create-only" && planned.current_hash !== null) {
      diagnostics.push(diagnostic7("HKT468", "Create-only assets cannot overwrite an existing file.", pointer));
      continue;
    }
    if (asset.update_strategy !== "three-way" && ["replace", "replace-candidate"].includes(planned.disposition)) {
      diagnostics.push(diagnostic7("HKT468", "Only three-way assets may be replaced from the template.", pointer));
      continue;
    }
    if (planned.disposition === "replace-candidate" && !approved.has(planned.asset_id)) {
      diagnostics.push(diagnostic7("HKT463", "Unowned replacement candidate requires explicit approval.", pointer));
      continue;
    }
    ...
    if (targetBytes !== undefined && !Buffer.isBuffer(targetBytes)) {
      diagnostics.push(diagnostic7("HKT464", "Resolved asset content must be bytes.", pointer));
      continue;
    }
    if (targetBytes === undefined && ["merge", "regenerate"].includes(planned.disposition)) {
      diagnostics.push(diagnostic7("HKT464", `${planned.disposition} requires explicit resolved bytes.`, pointer));
      continue;
    }
    if (targetBytes === undefined) {
      if (asset.source === null) {
        diagnostics.push(diagnostic7("HKT464", "Asset has no source or resolved bytes.", pointer));
        continue;
      }
      try {
        targetBytes = await readSafeSource(templatesRoot, asset.source);
      } catch (error) {
        diagnostics.push(diagnostic7("HKT462", `Cannot safely read asset source: ${error.message}`, pointer));
        continue;
      }
    } else {
      targetBytes = Buffer.from(targetBytes);
    }
    if (!explicitlyResolved && planned.target_hash !== digest3(targetBytes)) {
      diagnostics.push(diagnostic7("HKT466", "Template source changed after the plan was created.", pointer));
      continue;
    }
    const operation = planned.current_hash === null ? "create" : planned.disposition === "merge" || ["structured-merge", "typed-migration", "create-only", "preserve"].includes(asset.update_strategy) ? "merge" : "replace";
    const backupPath = operation === "create" ? null : `.workflow/backups/${transactionId}/${planned.destination}`;
    operations.push({
      operation,
      path: planned.destination,
      before_hash: planned.current_hash,
      after_hash: digest3(targetBytes),
  ...
  return created.diagnostics.length ? { journal: null, contentByPath: null, modeByPath: null, diagnostics: created.diagnostics } : { journal: created.journal, journalBytes: created.bytes, contentByPath, modeByPath, diagnostics: [] };
}
// packages/core/src/transaction-preparer.js
import { createHash as createHash7, randomBytes as randomBytes3 } from "node:crypto";
import { constants as constants6 } from "node:fs";
import { chmod, lstat as lstat7, mkdir, open as open6, readFile as readFile3, readdir, realpath as realpath7, rename as rename2, rm, rmdir, unlink as unlink2 } from "node:fs/promises";
import path8 from "node:path";
var projectUpdateLocks = new WeakMap;
var TRANSACTION_ID_PATTERN = /^[0-9]{8}T[0-9]{6}Z-[0-9a-f]{16}$/;
function diagnostic8(code, message, pathValue = "") {
...
function safeRelative3(value) {
  if (typeof value !== "string" || path8.isAbsolute(value) || value.includes("\\"))
    return false;
  return value.split("/").every((part) => part && part !== "." && part !== ".." && !part.includes(":") && !/[<>"|?*]/.test(part) && !/[. ]$/.test(part) && !/[\u0000-\u001f]/.test(part) && !/^(con|prn|aux|nul|com[1-9]|lpt[1-9])(?:\.|$)/i.test(part));
}
async function inspectPath(root, relative) {
  if (!safeRelative3(relative))
    throw new Error("path is not a safe project-relative path");
  const parts = relative.split("/");
  let current = root;
  for (let index = 0;index < parts.length; index += 1) {
    ...
    if (index === parts.length - 1) {
      if (!metadata.isFile())
        throw new Error("path is not a regular file");
      const handle = await open6(current, constants6.O_RDONLY | (constants6.O_NOFOLLOW ?? 0));
      try {
        const openedMetadata = await handle.stat();
        if (!openedMetadata.isFile())
          throw new Error("path changed while it was opened");
        return { bytes: await handle.readFile(), mode: openedMetadata.mode & 511 };
      } finally {
        await handle.close();
      }
    }
...
async function inspectArtifactTree(root, relative) {
  if (!safeRelative3(relative))
    throw new Error("artifact directory is not a safe project-relative path");
  let current = root;
  for (const part of relative.split("/")) {
    ...
    if (metadata.isSymbolicLink() || !metadata.isDirectory())
      throw new Error(`unsafe artifact directory: ${relative}`);
  }
  let files = 0;
  let bytes = 0;
  const walk = async (directory) => {
    for (const entry of await readdir(directory, { withFileTypes: true })) {
      const destination = path8.join(directory, entry.name);
      const metadata = await lstat7(destination);
      if (metadata.isSymbolicLink())
      ...
      if (metadata.isDirectory())
        ...
        throw new Error(`artifact tree contains a special file: ${relative}`);
    }
  };
  await walk(current);
...
async function ensureDirectory(root, relative) {
  if (!safeRelative3(relative))
    throw new Error("directory is not a safe project-relative path");
  let current = root;
  for (const part of relative.split("/")) {
    current = path8.join(current, part);
    try {
      ...
      if (metadata.isSymbolicLink() || !metadata.isDirectory())
        throw new Error(`unsafe transaction directory: ${relative}`);
    } catch (error) {
      if (error.code !== "ENOENT")
        throw error;
      let created = false;
      ...
      if (metadata.isSymbolicLink() || !metadata.isDirectory())
        throw new Error(`unsafe transaction directory: ${relative}`);
      if (created)
        await syncDirectory(path8.dirname(current));
    }
  }
  return current;
}
async function createExclusiveDirectory(root, relative) {
  const parent = path8.posix.dirname(relative);
  if (parent !== ".")
    await ensureDirectory(root, parent);
  const destination = path8.join(root, ...relative.split("/"));
  await mkdir(destination, { mode: 448 });
  await chmod(destination, 448);
  await syncDirectory(path8.dirname(destination));
  return destination;
}
async function syncDirectory(directory) {
  let handle;
  try {
    handle = await open6(directory, constants6.O_RDONLY);
    await handle.sync();
  } catch (error) {
    if (!["EINVAL", "ENOTSUP", "EPERM", "EISDIR"].includes(error.code))
      throw error;
...
async function writeExclusive(root, relative, bytes, mode = 384) {
  const parentRelative = path8.posix.dirname(relative);
  if (parentRelative !== ".")
    await ensureDirectory(root, parentRelative);
  const destination = path8.join(root, ...relative.split("/"));
  const handle = await open6(destination, constants6.O_CREAT | constants6.O_EXCL | constants6.O_WRONLY | (constants6.O_NOFOLLOW ?? 0), mode);
  try {
    await handle.writeFile(bytes);
    await handle.sync();
  } finally {
...
async function atomicPublish(root, relative, bytes) {
  ...
  if (parentRelative !== ".")
    await ensureDirectory(root, parentRelative);
  const destination = path8.join(root, ...relative.split("/"));
  const temporary = `${destination}.tmp-${process.pid}-${randomBytes3(8).toString("hex")}`;
  let handle;
  let published = false;
  try {
    handle = await open6(temporary, constants6.O_CREAT | constants6.O_EXCL | constants6.O_WRONLY, 384);
    await handle.writeFile(bytes);
    await handle.sync();
    await handle.close();
    handle = null;
    try {
      await lstat7(destination);
      throw new Error(`transaction artifact already exists: ${relative}`);
    } catch (error) {
      if (error.code !== "ENOENT")
        throw error;
    }
...
async function atomicReplace(root, relative, bytes, mode = 384) {
  const existing = await inspectPath(root, relative);
  if (existing === null)
    throw new Error(`transaction artifact is missing: ${relative}`);
  const destination = path8.join(root, ...relative.split("/"));
  const temporary = `${destination}.tmp-${process.pid}-${randomBytes3(8).toString("hex")}`;
  let handle;
  let replaced = false;
  try {
    handle = await open6(temporary, constants6.O_CREAT | constants6.O_EXCL | constants6.O_WRONLY, mode);
    await handle.writeFile(bytes);
    await handle.sync();
    await handle.close();
    handle = null;
...
async function atomicInstall(root, relative, bytes, mode) {
  const destination = path8.join(root, ...relative.split("/"));
  const temporary = `${destination}.hekate-${process.pid}-${randomBytes3(8).toString("hex")}`;
  let handle;
  let installed = false;
  try {
    handle = await open6(temporary, constants6.O_CREAT | constants6.O_EXCL | constants6.O_WRONLY | (constants6.O_NOFOLLOW ?? 0), mode);
    await handle.writeFile(bytes);
    await handle.sync();
    await handle.close();
    handle = null;
...
async function missingParentPaths(root, operationPaths) {
  ...
  for (const operationPath of operationPaths) {
    const parent = path8.posix.dirname(operationPath);
    if (parent === ".")
      continue;
    let current = root;
    ...
    for (const part of parent.split("/")) {
      ...
      try {
        const metadata = await lstat7(current);
        if (metadata.isSymbolicLink() || !metadata.isDirectory())
          throw new Error(`unsafe operation parent: ${relative}`);
      } catch (error) {
        if (error.code !== "ENOENT")
          throw error;
        parentMissing = true;
...
function orderedOperations(journal, prepared) {
  return journal.operations.map((operation, index) => ({ operation, metadata: prepared.operations[index], index })).sort((left, right) => {
...
function validateApplyState(applyState, transactionId, journal) {
  ...
  if (diagnostics.length || applyState.transaction_id !== transactionId)
    throw new Error("apply state is invalid");
  ...
  if (applyState.created_parent_paths.some((item) => !safeRelative3(item) || !allowed.has(item)))
    throw new Error("apply state contains an unrelated parent path");
  return applyState;
}
function plannedJournalHash(journal) {
  return hash4(canonicalBytes(journalWithStatus(journal, "planned")));
}
async function loadPreparedTransaction(root, transactionId) {
  ...
  if (!journalFile || !preparedFile)
    throw new Error("prepared transaction artifacts are missing");
  let journal;
  let prepared;
  try {
    ...
    throw new Error("prepared transaction artifacts are malformed JSON");
  }
  const diagnostics = [...validateOperationJournal(journal), ...validatePreparedTransaction(prepared)];
  if (diagnostics.length)
    throw new Error(`prepared transaction is invalid: ${diagnostics[0].message}`);
  if (journal.transaction_id !== transactionId || prepared.transaction_id !== transactionId)
    throw new Error("prepared transaction identity does not match its path");
  if (prepared.journal_hash !== plannedJournalHash(journal))
    throw new Error("prepared transaction journal hash does not match");
  if (journal.operations.length !== prepared.operations.length)
    throw new Error("prepared operation count does not match the journal");
  for (let index = 0;index < journal.operations.length; index += 1) {
    ...
    for (const key of ["operation", "path", "ownership", "before_hash", "after_hash", "backup_path"]) {
      if (left[key] !== right[key])
        throw new Error("prepared operation does not match the journal");
    }
  }
...
async function verifyPreparedBytes(root, journal, prepared) {
  ...
  for (let index = 0;index < journal.operations.length; index += 1) {
    ...
    } else if (!current || hash4(current.bytes) !== operation.before_hash) {
    ...
    if (metadata.staged_path !== null) {
      ...
      if (!staged || hash4(staged.bytes) !== operation.after_hash)
        throw new Error(`staged bytes no longer match after_hash: ${operation.path}`);
    }
    let backup = null;
    if (operation.backup_path !== null) {
      backup = await inspectPath(root, operation.backup_path);
      if (!backup || hash4(backup.bytes) !== operation.before_hash)
        throw new Error(`backup no longer matches before_hash: ${operation.path}`);
    }
...
function parseArtifact(bytes, kind, file) {
  if (!Buffer.isBuffer(bytes))
    return { value: null, diagnostics: [diagnostic8("HKT446", `${file} must be supplied as bytes.`)] };
  let value;
  try {
    value = JSON.parse(bytes.toString("utf8"));
  } catch {
    return { value: null, diagnostics: [diagnostic8("HKT446", `${file} is malformed JSON.`)] };
  }
...
function validatePreparedTransaction(value) {
  ...
  for (const [index, operation] of value.operations.entries()) {
    const pointer = `/operations/${index}`;
    if (!safeRelative3(operation.path))
      diagnostics.push(diagnostic8("HKT447", "Prepared operation path is unsafe.", `${pointer}/path`));
    if ([".workflow/backups", ".workflow/migration", ".workflow/transactions"].some((prefix) => operation.path === prefix || operation.path.startsWith(`${prefix}/`)) || operation.path === ".workflow/update.lock") {
      diagnostics.push(diagnostic8("HKT447", "Prepared operation targets transaction-owned state.", `${pointer}/path`));
    }
    const key = operation.path.toLowerCase();
    if (paths.has(key))
      diagnostics.push(diagnostic8("HKT447", "Prepared operation paths collide.", `${pointer}/path`));
    paths.add(key);
    ...
    if (operation.backup_path !== expectedBackup)
      diagnostics.push(diagnostic8("HKT447", "Prepared backup path is not derived from the transaction.", `${pointer}/backup_path`));
    if (operation.staged_path !== expectedStage)
      diagnostics.push(diagnostic8("HKT447", "Prepared staged path is not derived from the transaction.", `${pointer}/staged_path`));
    const shapeIsValid = operation.operation === "create" ? operation.before_hash === null && operation.before_mode === null && operation.after_hash !== null && operation.target_mode !== null : operation.operation === "delete" ? operation.before_hash !== null && operation.before_mode !== null && operation.after_hash === null && operation.target_mode === null : operation.before_hash !== null && operation.before_mode !== null && operation.after_hash !== null && operation.target_mode !== null;
    if (!shapeIsValid)
      diagnostics.push(diagnostic8("HKT447", "Prepared operation metadata is incomplete.", pointer));
  }
  ...
  if (value.operations.some((operation, index) => operation.path !== sortedPaths[index])) {
    diagnostics.push(diagnostic8("HKT447", "Prepared operations are not in canonical path order.", "/operations"));
  }
  return diagnostics;
}
async function acquireProjectUpdateLock({ root, transactionId }) {
  if (!TRANSACTION_ID_PATTERN.test(transactionId))
    throw new TypeError("invalid transaction ID");
  const projectRoot = await realpath7(root);
  const workflow = await lstat7(path8.join(projectRoot, ".workflow"));
  if (workflow.isSymbolicLink() || !workflow.isDirectory())
    throw new Error(".workflow must be a real directory");
  const relative = ".workflow/update.lock";
  const destination = path8.join(projectRoot, ".workflow", "update.lock");
  const token = canonicalBytes({ schema_version: 1, transaction_id: transactionId, pid: process.pid });
  let handle;
  let created = false;
  try {
    handle = await open6(destination, constants6.O_CREAT | constants6.O_EXCL | constants6.O_WRONLY | (constants6.O_NOFOLLOW ?? 0), 384);
    created = true;
    ...
    if (handle)
      await handle.close().catch(() => {});
    if (created) {
      await unlink2(destination).catch(() => {});
      await syncDirectory(path8.dirname(destination)).catch(() => {});
    }
    throw error;
  }
  ...
  const lock = {
    ...
    async release() {
      if (state.released)
        return;
      const current = await readFile3(destination);
      if (!current.equals(token))
        throw new Error("project update lock changed ownership");
      await unlink2(destination);
      await syncDirectory(path8.dirname(destination));
      state.released = true;
    }
  };
...
async function recoverProjectUpdateLock({ root, transactionId }) {
  if (!TRANSACTION_ID_PATTERN.test(transactionId))
    throw new TypeError("invalid transaction ID");
  const projectRoot = await realpath7(root);
  ...
  let lock;
  try {
    lock = JSON.parse(lockFile.bytes.toString("utf8"));
  } catch {
  ...
  await unlink2(path8.join(projectRoot, ".workflow", "update.lock"));
  await syncDirectory(path8.join(projectRoot, ".workflow"));
  return true;
}
async function writeMigrationReport({ root, runId, importBytes = null, reportBytes = null }) {
  if (importBytes === null && reportBytes === null)
    return { written: null, diagnostics: [] };
  const projectRoot = await realpath7(root);
  const directory = `.workflow/migration/${runId}`;
  await createExclusiveDirectory(projectRoot, directory);
  if (importBytes !== null)
    await writeExclusive(projectRoot, `${directory}/import.json`, importBytes);
  if (reportBytes !== null)
    await writeExclusive(projectRoot, `${directory}/report.json`, reportBytes);
  return { written: directory, diagnostics: [] };
}
async function prepareOperationTransaction({ root, journal, contentByPath = new Map, modeByPath = new Map, importBytes = null, reportBytes = null, recoveryFiles = new Map, updateLock = null }) {
  journal = structuredClone(journal);
  contentByPath = contentByPath instanceof Map ? new Map([...contentByPath].map(([key, bytes]) => [key, Buffer.isBuffer(bytes) ? Buffer.from(bytes) : bytes])) : contentByPath;
  modeByPath = modeByPath instanceof Map ? new Map(modeByPath) : modeByPath;
  if (Buffer.isBuffer(importBytes))
    importBytes = Buffer.from(importBytes);
  if (Buffer.isBuffer(reportBytes))
    reportBytes = Buffer.from(reportBytes);
  recoveryFiles = recoveryFiles instanceof Map ? new Map([...recoveryFiles].map(([key, bytes]) => [key, Buffer.isBuffer(bytes) ? Buffer.from(bytes) : bytes])) : recoveryFiles;
  const diagnostics = validateOperationJournal(journal);
  if (diagnostics.length)
    return { prepared: null, diagnostics };
  if (journal.status !== "planned")
    return { prepared: null, diagnostics: [diagnostic8("HKT440", "Only a planned journal can be prepared.", "/status")] };
  if (!(contentByPath instanceof Map) || !(modeByPath instanceof Map)) {
    return { prepared: null, diagnostics: [diagnostic8("HKT440", "Transaction content and modes must be supplied as maps.")] };
  }
  if (!(recoveryFiles instanceof Map))
    return { prepared: null, diagnostics: [diagnostic8("HKT440", "Recovery runtime files must be supplied as a map.")] };
  for (const [relative, bytes] of recoveryFiles) {
    if (!/^(?:SHA256SUMS|src\/hekate-cli\.mjs|schemas\/[a-z-]+\.schema\.json)$/.test(relative) || !Buffer.isBuffer(bytes)) {
      diagnostics.push(diagnostic8("HKT440", "Recovery runtime contains an invalid file.", relative));
    }
  }
  if (importBytes === null !== (reportBytes === null)) {
    return { prepared: null, diagnostics: [diagnostic8("HKT446", "Private import and public report artifacts must be supplied together.")] };
  }
  if (importBytes !== null) {
    ...
    if (report.value)
      diagnostics.push(...validateMigrationReport(report.value));
    if (diagnostics.length)
      return { prepared: null, diagnostics };
  }
  let projectRoot;
  try {
    projectRoot = await realpath7(root);
  } catch (error) {
    return { prepared: null, diagnostics: [diagnostic8("HKT440", `Cannot resolve transaction root: ${error.message}`)] };
  }
  ...
  for (const key of contentByPath.keys()) {
    if (!expectedContent.has(key))
      diagnostics.push(diagnostic8("HKT442", "Transaction contains unplanned staged content.", key));
  }
  if (diagnostics.length)
    return { prepared: null, diagnostics };
  let lock;
  ...
  try {
    ...
    return { prepared: null, diagnostics: [diagnostic8("HKT444", `Cannot acquire project update lock: ${error.message}`)] };
  }
  try {
    ...
    for (const [index, operation] of journal.operations.entries()) {
      ...
      if (operation.backup_path !== expectedBackup)
        diagnostics.push(diagnostic8("HKT443", "Backup path does not belong to this transaction.", `${pointer}/backup_path`));
      let current;
      try {
        current = await inspectPath(projectRoot, operation.path);
      } catch (error) {
        diagnostics.push(diagnostic8("HKT441", `Cannot safely inspect operation target: ${error.message}`, `${pointer}/path`));
        continue;
      }
      if (operation.operation === "create") {
        if (current !== null)
          diagnostics.push(diagnostic8("HKT442", "Create target already exists.", `${pointer}/path`));
      } else if (current === null) {
        diagnostics.push(diagnostic8("HKT442", "Operation target is missing.", `${pointer}/path`));
      } else if (hash4(current.bytes) !== operation.before_hash) {
        diagnostics.push(diagnostic8("HKT442", "Operation target no longer matches before_hash.", `${pointer}/before_hash`));
      }
      const stagedBytes = contentByPath.get(operation.path);
      if (operation.operation !== "delete") {
        if (!Buffer.isBuffer(stagedBytes))
          diagnostics.push(diagnostic8("HKT442", "Operation is missing staged bytes.", `${pointer}/after_hash`));
        else if (hash4(stagedBytes) !== operation.after_hash)
          diagnostics.push(diagnostic8("HKT442", "Staged bytes do not match after_hash.", `${pointer}/after_hash`));
      }
      const requestedMode = modeByPath.get(operation.path);
      if (requestedMode !== undefined && (!Number.isInteger(requestedMode) || requestedMode < 0 || requestedMode > 511)) {
        diagnostics.push(diagnostic8("HKT442", "Target mode must be an integer between 0000 and 0777.", pointer));
      }
      inspected.set(operation.path, { current, stagedBytes, targetMode: requestedMode ?? current?.mode ?? 384 });
    }
    if (diagnostics.length)
      return { prepared: null, diagnostics };
    ...
    const preparingMarker = canonicalBytes({
      ...
      journal_hash: hash4(journalBytes)
    });
    ...
    for (const [relative, bytes] of recoveryFiles) {
      await writeExclusive(projectRoot, `${transactionBase}/runtime/${relative}`, bytes, relative === "src/hekate-cli.mjs" ? 448 : 384);
    }
    if (journal.operations.some((operation) => operation.operation !== "create"))
      await createExclusiveDirectory(projectRoot, backupBase);
    if (importBytes !== null)
      await createExclusiveDirectory(projectRoot, `.workflow/migration/${journal.transaction_id}`);
    ...
    for (const operation of journal.operations) {
      ...
      if (operation.backup_path !== null)
        await writeExclusive(projectRoot, operation.backup_path, current.bytes);
      if (stagedPath !== null)
        await writeExclusive(projectRoot, stagedPath, stagedBytes);
    ...
    if (preparedDiagnostics.length)
      return { prepared: null, diagnostics: preparedDiagnostics };
    ...
    await unlink2(path8.join(projectRoot, ...`${transactionBase}/preparing.json`.split("/"))).catch(() => {});
    await syncDirectory(path8.join(projectRoot, ...transactionBase.split("/"))).catch(() => {});
    return {
      prepared,
    ...
    return { prepared: null, diagnostics: [diagnostic8("HKT445", `Cannot prepare transaction: ${error.message}`)] };
  } finally {
    if (ownsLock)
      await lock.release();
  }
}
async function applyPreparedTransaction({ root, transactionId, verify, operationApplied = null, updateLock = null }) {
  if (typeof verify !== "function") {
    return { applied: false, diagnostics: [diagnostic8("HKT450", "Apply requires a target verification function.")] };
  }
  let projectRoot;
  try {
    projectRoot = await realpath7(root);
  } catch (error) {
    return { applied: false, diagnostics: [diagnostic8("HKT450", `Cannot resolve transaction root: ${error.message}`)] };
  }
  let lock;
  let ownsLock = false;
  try {
    ...
    return { applied: false, diagnostics: [diagnostic8("HKT444", `Cannot acquire project update lock: ${error.message}`)] };
  }
  let loaded;
  let journalPath;
  try {
    loaded = await loadPreparedTransaction(projectRoot, transactionId);
    if (loaded.journal.status !== "planned")
      throw new Error(`transaction is not planned: ${loaded.journal.status}`);
    ...
    const createdParents = await missingParentPaths(projectRoot, loaded.journal.operations.filter((operation) => operation.operation !== "delete").map((operation) => operation.path));
    ...
    for (const relative of createdParents) {
      const destination = path8.join(projectRoot, ...relative.split("/"));
      await mkdir(destination, { mode: 493 });
      await syncDirectory(path8.dirname(destination));
    }
    ...
    for (const { operation, metadata } of ordered) {
      ...
      if (operation.operation === "create") {
        if (current !== null)
          throw new Error(`create target changed during apply: ${operation.path}`);
      } else if (!current || hash4(current.bytes) !== operation.before_hash || current.mode !== metadata.before_mode) {
        throw new Error(`operation target changed during apply: ${operation.path}`);
      }
      if (operation.operation === "delete") {
        await unlink2(path8.join(projectRoot, ...operation.path.split("/")));
        await syncDirectory(path8.dirname(path8.join(projectRoot, ...operation.path.split("/"))));
      } else {
        await atomicInstall(projectRoot, operation.path, material.get(operation.path).staged.bytes, metadata.target_mode);
      }
      if (operationApplied)
        await operationApplied({ operation: structuredClone(operation), root: projectRoot });
    }
    ...
    return { applied: false, diagnostics: [diagnostic8("HKT451", `Cannot apply transaction: ${error.message}`), ...error.verificationDiagnostics ?? []] };
  } finally {
    if (ownsLock)
      await lock.release();
  }
}
async function cleanupTransactionBundle({ root, transactionId, dryRun = false, removeArtifact = null }) {
  if (!TRANSACTION_ID_PATTERN.test(transactionId)) {
    return { cleaned: false, status: null, actions: [], removed: [], diagnostics: [diagnostic8("HKT454", "Cannot clean transaction: invalid transaction ID")] };
  }
  let projectRoot;
  try {
    projectRoot = await realpath7(root);
  } catch (error) {
    return { cleaned: false, status: null, actions: [], removed: [], diagnostics: [diagnostic8("HKT454", `Cannot resolve cleanup root: ${error.message}`)] };
  }
  let lock = null;
  if (dryRun) {
    try {
      if (await inspectPath(projectRoot, ".workflow/update.lock")) {
        return { cleaned: false, status: null, actions: [], removed: [], diagnostics: [diagnostic8("HKT444", "Cannot inspect cleanup while the project update lock is held.")] };
      }
    } catch (error) {
      return { cleaned: false, status: null, actions: [], removed: [], diagnostics: [diagnostic8("HKT444", `Cannot inspect project update lock: ${error.message}`)] };
    }
  } else {
    try {
      lock = await acquireProjectUpdateLock({ root: projectRoot, transactionId });
    } catch (error) {
      return { cleaned: false, status: null, actions: [], removed: [], diagnostics: [diagnostic8("HKT444", `Cannot acquire project update lock: ${error.message}`)] };
    }
  }
  let status = null;
  let actions = [];
  ...
  try {
    ...
    if (!trees.size) {
      ...
      if (trees.has(transactionBase)) {
        ...
        if (journal) {
          ...
          if (!["committed", "rolled_back"].includes(status))
            throw new Error(`transaction cannot be cleaned from status: ${status}`);
        } else {
          const markerFile = await inspectPath(projectRoot, `${transactionBase}/preparing.json`);
          if (!markerFile)
            throw new Error("transaction journal is missing without an unpublished preparation marker");
          ...
          if (keys.join(",") !== "journal_hash,schema_version,transaction_id" || marker.schema_version !== 1 || marker.transaction_id !== transactionId || !/^sha256:[0-9a-f]{64}$/.test(marker.journal_hash) || !canonicalBytes(marker).equals(markerFile.bytes)) {
      ...
      if (dryRun) {
        ...
        for (const action of actions) {
          ...
          if (!tree)
            continue;
          if (removeArtifact)
            await removeArtifact({ path: action.path, absolute: tree.absolute });
          else
            await rm(tree.absolute, { recursive: true, force: false, maxRetries: 3, retryDelay: 100 });
          await syncDirectory(path8.dirname(tree.absolute));
    ...
    result = { cleaned: false, status, actions, removed, diagnostics: [diagnostic8("HKT454", `Cannot clean transaction: ${error.message}`)] };
  }
  if (lock) {
    try {
      await lock.release();
      ...
      result.diagnostics.push(diagnostic8("HKT444", `Cannot release project update lock: ${error.message}`));
    }
  }
  return result;
}
async function rollbackPreparedTransaction({ root, transactionId, dryRun = false, operationRolledBack = null }) {
  let projectRoot;
  try {
    projectRoot = await realpath7(root);
  } catch (error) {
    return { rolled_back: false, conflicts: [], actions: [], diagnostics: [diagnostic8("HKT453", `Cannot resolve transaction root: ${error.message}`)] };
  }
  let lock = null;
  if (dryRun) {
    try {
      if (await inspectPath(projectRoot, ".workflow/update.lock")) {
        return { rolled_back: false, conflicts: [], actions: [], diagnostics: [diagnostic8("HKT444", "Cannot inspect rollback while the project update lock is held.")] };
      }
    } catch (error) {
      return { rolled_back: false, conflicts: [], actions: [], diagnostics: [diagnostic8("HKT444", `Cannot inspect project update lock: ${error.message}`)] };
    }
  } else {
    try {
      lock = await acquireProjectUpdateLock({ root: projectRoot, transactionId });
    } catch (error) {
      return { rolled_back: false, conflicts: [], actions: [], diagnostics: [diagnostic8("HKT444", `Cannot acquire project update lock: ${error.message}`)] };
    }
  }
  let loaded;
  let journalPath;
  try {
    ...
    const applyState = applyStateFile ? validateApplyState(JSON.parse(applyStateFile.bytes.toString("utf8")), transactionId, loaded.journal) : { schema_version: 1, transaction_id: transactionId, created_parent_paths: [] };
    const actions = [];
    const conflicts = [];
    const material = new Map;
    for (let index = 0;index < loaded.journal.operations.length; index += 1) {
      ...
      try {
        ...
        conflicts.push({ path: operation.path, reason: error.message });
        continue;
      }
      const currentHash = current ? hash4(current.bytes) : null;
      if (operation.operation === "create") {
        if (currentHash === operation.after_hash && current.mode === metadata.target_mode)
          actions.push({ operation: "remove", path: operation.path, metadata });
        else if (current !== null)
          conflicts.push({ path: operation.path, reason: "current bytes do not match after_hash" });
        ...
        if (current === null)
          ...
          conflicts.push({ path: operation.path, reason: "deleted path was recreated or changed" });
    ...
    for (const action of actions.filter((item) => item.operation === "restore")) {
      ...
      if (!backup || hash4(backup.bytes) !== action.metadata.before_hash)
        throw new Error(`rollback backup is invalid: ${action.path}`);
      material.set(action.path, backup.bytes);
    }
    if (dryRun)
      return { rolled_back: false, conflicts, actions: actions.map(({ operation, path: actionPath }) => ({ operation, path: actionPath })), diagnostics: [] };
    if (conflicts.length) {
    ...
    const rollbackActions = orderedOperations(loaded.journal, loaded.prepared).reverse().map(({ operation }) => actionByPath.get(operation.path)).filter(Boolean);
    for (const action of rollbackActions) {
      const destination = path8.join(projectRoot, ...action.path.split("/"));
      const current = await inspectPath(projectRoot, action.path);
      if (action.operation === "remove") {
        if (!current || hash4(current.bytes) !== action.metadata.after_hash || current.mode !== action.metadata.target_mode)
          throw new Error(`rollback target changed: ${action.path}`);
        await unlink2(destination);
        await syncDirectory(path8.dirname(destination));
      } else {
        const journalOperation = loaded.journal.operations.find((operation) => operation.path === action.path);
        const expectedCurrent = journalOperation.operation === "delete" ? current === null : current && hash4(current.bytes) === action.metadata.after_hash && current.mode === action.metadata.target_mode;
        if (!expectedCurrent)
          throw new Error(`rollback target changed: ${action.path}`);
        await atomicInstall(projectRoot, action.path, material.get(action.path), action.metadata.before_mode);
      }
      if (operationRolledBack)
        await operationRolledBack({ operation: action.operation, path: action.path, root: projectRoot });
    }
    for (const relative of [...applyState.created_parent_paths].sort((left, right) => right.split("/").length - left.split("/").length || right.localeCompare(left, "en"))) {
      await rmdir(path8.join(projectRoot, ...relative.split("/"))).catch((error) => {
    ...
    return { rolled_back: false, conflicts: [], actions: [], diagnostics: [diagnostic8("HKT453", `Cannot roll back transaction: ${error.message}`)] };
  } finally {
    if (lock)
      await lock.release();
  }
...
// packages/cli/src/main.js
var packagedPayload = new URL("../payload/", import.meta.url);
var defaultSourceRoot = existsSync(new URL("distribution/install-manifest.json", packagedPayload)) ? fileURLToPath(packagedPayload) : fileURLToPath(new URL("../../../", import.meta.url));
var schemaNames = ["apply-state", "config", "install-state", "journal", "legacy-import", "lock", "manifest", "migration-report", "plan", "prepared-transaction", "project"];
async function loadRecoveryRuntime() {
  ...
  const payload = new Map(await Promise.all(files.map(async (relative) => [relative, await readFile4(path9.join(runtimeRoot, ...relative.split("/")))])));
  const checksumBytes = await readFile4(path9.join(runtimeRoot, "SHA256SUMS"));
...
`).map((line) => {
  ...
  if (checksums.size !== files.length || files.some((relative) => !checksums.has(relative)))
    throw new Error("SHA256SUMS inventory is incomplete");
  for (const [relative, bytes] of payload) {
    if (createHash8("sha256").update(bytes).digest("hex") !== checksums.get(relative))
      throw new Error(`runtime checksum mismatch: ${relative}`);
  }
...
function diagnostic9(code, message, file = "") {
  return { code, severity: "error", file, path: "", message };
}
...
`);
    for (const item of result.diagnostics)
      process.stdout.write(`${item.code} ${item.file}${item.path ? ` ${item.path}` : ""}: ${item.message}
`);
    if (command === "compile")
      process.stdout.write(result.wrote ? `wrote status.lock.json
...
function parseUpgrade(args) {
  ...
  for (let index = 0;index < args.length; index += 1) {
    ...
    if (match && options[match[1]] === null && match[2]) {
...
`);
    if (result.recovery_runtime)
      process.stdout.write(`offline recovery runtime: ${result.recovery_runtime}
`);
    if (result.plan)
      process.stdout.write(result.plan);
    for (const [operation, count] of Object.entries(result.summary))
...
`);
  if (result.ok)
    process.stdout.write(result.dry_run ? `dry run; no files changed
` : `upgrade committed
`);
}
function formatUpgradePlan({ summary, preservation, unresolvedCritical = preservation?.unresolved ?? 0, unownedPaths, migratedPaths, transactionId }) {
  const lines = [];
  if (preservation) {
    ...
    if (preservation.unresolved > unresolvedCritical)
      lines.push(`unresolved non-critical: ${preservation.unresolved - unresolvedCritical} settings`);
  }
  ...
  for (const relativePath of migratedPaths)
    lines.push(`migrate: ${relativePath}`);
  for (const relativePath of unownedPaths)
    lines.push(`replace unowned: ${relativePath}`);
  return `${lines.join(`
`)}
`;
}
async function confirmUpgrade(summary, plan) {
  ...
  try {
    return /^y(?:es)?$/i.test((await terminal.question(`Apply ${total} transactional file operation(s)? [y/N] `)).trim());
  } finally {
    terminal.close();
  }
}
async function upgrade(args, invocationRoot) {
  ...
  if (!options || !options.to || !options.force)
    return { usage: true, exitCode: 2 };
  const root = path9.resolve(options.target ?? invocationRoot);
  const sourceRoot = path9.resolve(options.source ?? defaultSourceRoot);
  const loadedManifest = await loadInstallManifest(pathToFileURL(path9.join(sourceRoot, "distribution/install-manifest.json")));
  if (!loadedManifest.manifest)
    return { result: { command: "upgrade", ok: false, dry_run: options.dryRun, transaction_id: null, summary: null, diagnostics: loadedManifest.diagnostics }, json: options.json, exitCode: 1 };
  const manifest = loadedManifest.manifest;
  const transactionId = createTransactionId();
  let updateLock = null;
  if (!options.dryRun) {
    try {
      updateLock = await acquireProjectUpdateLock({ root, transactionId });
    } catch (error) {
      return { result: { command: "upgrade", ok: false, dry_run: false, transaction_id: transactionId, summary: null, diagnostics: [diagnostic9("HKT444", `Cannot acquire project update lock: ${error.message}`)] }, json: options.json, exitCode: 1 };
    }
  }
  try {
    const loadedState = await loadInstalledState(root, manifest);
    if (loadedState.diagnostics.length)
      return { result: { command: "upgrade", ok: false, dry_run: options.dryRun, transaction_id: null, summary: null, diagnostics: loadedState.diagnostics }, json: options.json, exitCode: 1 };
    let imported = null;
    let report = null;
    let importBytes = null;
    let reportBytes = null;
    if (!loadedState.state) {
      const hasTypedSource = [".workflow/config.yml", ".workflow/project.yml"].some((relative) => existsSync(path9.join(root, relative)));
      if (hasTypedSource) {
    ...
    const adapters = options.adapters === null ? loadedState.state?.selected_adapters ?? imported?.selected_adapters ?? [] : options.adapters.split(",").filter(Boolean);
    const optionalComponentIds = new Set(manifest.components.filter((component) => !component.default && component.adapters.length === 0).map((component) => component.id));
    const components = options.components === null ? (loadedState.state?.selected_components ?? []).filter((component) => optionalComponentIds.has(component)) : options.components.split(",").filter(Boolean);
    const planned = await planInstallation({ root, sourceRoot, adapters, components, manifest, installedState: loadedState.state });
    if (!planned.plan || planned.diagnostics.length)
      return { result: { command: "upgrade", ok: false, dry_run: options.dryRun, transaction_id: null, summary: null, preservation: report?.summary ?? null, diagnostics: planned.diagnostics }, json: options.json, exitCode: 1 };
    const targetImport = imported ? structuredClone(imported) : null;
    if (targetImport)
      targetImport.project.confirmation.state = "confirmed";
    ...
    if (!materialized.resolvedContentByAssetId)
      return { result: { command: "upgrade", ok: false, dry_run: options.dryRun, transaction_id: null, summary: null, preservation: report?.summary ?? null, diagnostics: materialized.diagnostics }, json: options.json, exitCode: 1 };
    ...
    const migratedPaths = planned.plan.assets.filter((asset) => asset.ownership === "project-seed" && asset.disposition !== "current").map((asset) => asset.destination).sort();
    ...
    if (!resolved.journal)
      return { result: { command: "upgrade", ok: false, dry_run: options.dryRun, transaction_id: transactionId, summary: null, preservation: report?.summary ?? null, diagnostics: resolved.diagnostics }, json: options.json, exitCode: 1 };
    const summary = { create: 0, replace: 0, merge: 0, delete: 0 };
    for (const operation of resolved.journal.operations)
    ...
    if (options.dryRun)
      return { result: { ...base, migration_report: null }, json: options.json, exitCode: 0 };
    if (options.yes && unownedPaths.length && !options.replaceUnowned) {
      return {
        result: { ...base, ok: false, diagnostics: [diagnostic9("HKT903", `Unowned files would be replaced without review: ${unownedPaths.join(", ")}. Re-run interactively or add --replace-unowned.`)] },
        json: options.json,
        exitCode: 1
      };
    }
    if (!options.yes && !await confirmUpgrade(summary, plan)) {
      return { result: { ...base, ok: false, diagnostics: [diagnostic9("HKT902", "Upgrade confirmation is required; use --yes for non-interactive execution.")] }, json: options.json, exitCode: 1 };
    }
    let recoveryFiles;
    try {
      recoveryFiles = await loadRecoveryRuntime();
    } catch (error) {
      return { result: { ...base, ok: false, diagnostics: [diagnostic9("HKT904", `Cannot load offline recovery runtime: ${error.message}`)] }, json: options.json, exitCode: 1 };
    }
    ...
    if (!prepared.prepared)
      return { result: { ...base, ok: false, diagnostics: prepared.diagnostics }, json: options.json, exitCode: 1 };
...
async function rollback(args, invocationRoot) {
  ...
  if (!options.dryRun && !options.yes) {
    return {
      result: { command: "rollback", ok: false, rolled_back: false, transaction_id: options.transaction, actions: [], conflicts: [], diagnostics: [diagnostic9("HKT902", "Rollback confirmation is required; use --yes for non-interactive execution.")] },
      json: options.json,
      exitCode: 1
    };
  }
  ...
  if (!options.dryRun) {
    try {
      await recoverProjectUpdateLock({ root, transactionId: options.transaction });
    } catch (error) {
      return {
        result: { command: "rollback", ok: false, rolled_back: false, transaction_id: options.transaction, actions: [], conflicts: [], diagnostics: [diagnostic9("HKT444", `Cannot recover project update lock: ${error.message}`)] },
        json: options.json,
        exitCode: 1
      };
    }
...
`);
  ...
  for (const action of result.actions)
    process.stdout.write(`${action.operation}: ${action.path}
`);
  for (const conflict of result.conflicts)
    process.stdout.write(`conflict: ${conflict.path}: ${conflict.reason}
`);
  for (const item of result.diagnostics)
    process.stdout.write(`${item.code}: ${item.message}
`);
  if (result.rolled_back)
    process.stdout.write(`rollback committed
`);
}
async function cleanup(args, invocationRoot) {
  ...
  if (!options.dryRun && !options.yes) {
    return {
      result: { command: "cleanup", ok: false, dry_run: false, cleaned: false, transaction_id: options.transaction, status: null, actions: [], removed: [], diagnostics: [diagnostic9("HKT902", "Cleanup confirmation is required; use --yes for non-interactive execution.")] },
      json: options.json,
      exitCode: 1
    };
  }
  ...
  if (!options.dryRun) {
    try {
      ...
      return {
        result: { command: "cleanup", ok: false, dry_run: false, cleaned: false, transaction_id: options.transaction, status: null, actions: [], removed: [], diagnostics: [diagnostic9("HKT444", `Cannot recover project update lock: ${error.message}`)] },
        json: options.json,
        exitCode: 1
      };
    }
...
`);
  ...
  for (const action of result.actions)
    process.stdout.write(`${action.operation}: ${action.path} (${action.files} files, ${action.bytes} bytes)
`);
  for (const item of result.diagnostics)
    process.stdout.write(`${item.code}: ${item.message}
`);
  if (result.ok)
    process.stdout.write(result.dry_run ? `dry run; offline rollback remains available
` : result.status === "absent" ? `transaction bundle already absent
` : `transaction bundle cleaned; offline rollback is no longer available
`);
}
async function main(args, root, loadAgent = null) {
  const command = args[0];
  if (command === "agent") {
    let agent;
    try {
      if (!loadAgent)
        throw new Error("this standalone transaction runtime does not include Pi");
...
`);
    ...
    const options2 = parseAgentOptions(args.slice(1));
    if (!options2) {
...
`);
    ...
    try {
      return await runAgent(options2, root);
    } catch (error) {
      process.stderr.write(`HKT950: ${error.message}
`);
      return 2;
    }
  }
...
`);
  ...
  try {
    ...
    if (options.has("--json"))
      process.stdout.write(`${JSON.stringify({ command, ok: false, gate_state: "invalid", diagnostics: [diagnostic9("HKT900", error.message)] })}
`);
    else
      process.stderr.write(`HKT900: ${error.message}
`);
